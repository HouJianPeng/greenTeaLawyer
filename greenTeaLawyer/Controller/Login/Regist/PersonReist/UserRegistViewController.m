...
- (void)viewDidLoad {
    ...
    [self.genderButton.layer setMasksToBounds:YES];
    [self.genderButton.layer setCornerRadius:5.0]; //设置矩圆角半径
    [self.genderButton.layer setBorderWidth:0.5];   //边框宽度
    self.genderButton.layer.borderColor = [UIColor lightGrayColor].CGColor;
    
    [self.idNumberTextField setBorderStyle:UITextBorderStyleRoundedRect];
    [self.borthDayTextField setBorderStyle:UITextBorderStyleRoundedRect];
    self.borthDayTextField.delegate = self;
    [self.officeTextField setBorderStyle:UITextBorderStyleRoundedRect];
}
...
//    NSString *extensionsStr = [self transformWithDictionary:extensionsDic];
    ...
    NSDictionary *parmater = @{@"mcode":self.verification,
             @"gender":self.genderButton.titleLabel.text,
...
- (BOOL)checkList
{
    if ([self.personNameTextField.text length]) {
        if (![self.genderButton.titleLabel.text isEqualToString: @"  性别"]) {
