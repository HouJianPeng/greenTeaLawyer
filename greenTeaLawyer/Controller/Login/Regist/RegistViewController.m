            ...
            if (![_verificationTextField.text length]) {
                ...
                [NetWork GET:CheckVerificationURL parmater:dic Block:^(NSData *data) {
                    ...
                    if ([r isEqualToNumber:[NSNumber numberWithInt:1]]) {
                        LawyerRegistViewController *lawyerRegistVC = [[LawyerRegistViewController alloc] init];
                        lawyerRegistVC.phoneNumber = self.phoneNumberTextField.text;
                        lawyerRegistVC.verification = self.verificationTextField.text;
                        lawyerRegistVC.password = self.setupPasswordTextField.text;
                        [self.navigationController pushViewController:lawyerRegistVC animated:YES];
                    }else{
