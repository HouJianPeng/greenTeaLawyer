...
+ (void)showAlertViewWithTitle:(NSString *)title
    ...
    [alert bk_setHandler:handlerBlock forButtonAtIndex:1];
    [alert show];
}

+ (void)showAlertViewLogOutHandlerBlock:(void(^)())handlerBlock {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"结束会话?" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    [alert bk_setHandler:^{
        handlerBlock();
    } forButtonAtIndex:1];
    [alert show];
